...
сам решает, когда уместно отправить селфи, или по команде `/photo`. Фото генерируются через Replicate (FLUX)
с постоянным описанием внешности, чтобы на всех снимках была одна и та же девушка.
...
3. Получите токен Replicate: https://replicate.com/account/api-tokens → `REPLICATE_API_TOKEN`.
4. В Railway: **New Project → Deploy from GitHub repo** → выберите этот репозиторий.
5. В **Variables** добавьте `TELEGRAM_BOT_TOKEN`, `ANTHROPIC_API_KEY`, `REPLICATE_API_TOKEN`.
6. (Рекомендуется) чтобы память не терялась при передеплое: **Add Volume** с путём `/data`
   и переменная `DATA_DIR=/data`.
7. Деплой запустится сам (`python bot.py`, long polling — домен и порт не нужны).
...
| `IMAGE_MODEL` | `black-forest-labs/flux-schnell` | Модель Replicate для фото |
...
Без `REPLICATE_API_TOKEN` бот работает, просто без фото.

## Локальный запуск
```bash
pip install -r requirements.txt
export TELEGRAM_BOT_TOKEN=... ANTHROPIC_API_KEY=... REPLICATE_API_TOKEN=...
python bot.py
```
